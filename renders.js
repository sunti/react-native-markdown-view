...
function renderTableCell(cell, row, column, rowCount, columnCount, output, state, styles, align) {
  ...
  contentStyle.push({ textAlign: align || 'center' })
    ...
    <Grid key={state.key} style={styles.table}>
      {[<Row id={1} key={1}>
        ...
        <Row id={row + 2} key={row + 2}>
          {cells.map((cell, column) => renderTableCell(cell, row + 2, column + 1, node.cells.length + 1, cells.length, output, state, styles, node.align[column]))}
        </Row>
      )))}
    </Grid>
  ),
